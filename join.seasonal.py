    ...
    for season in lseason:
        for varName in lvarName:
            ...
            for i,prjName in enumerate(lprjName):
                ...
                if prjName =="GSWP3":
                    srcPath = figDir + "/time.lat.%s.%s.%s.%s.%04d-%04d.png"%(calctype, "ref", varName, prjName, iYear,eYear)
                else:
                    srcPath = figDir + "/time.lat.%s.%s.%s.%s.%04d-%04d.png"%(calctype, "dif", varName, prjName, iYear,eYear)
            ...
            outPath  = figDir + "/join.time.lat.%s.dif.%04d-%04d.%s.png"%(calctype,iYear,eYear,varName)
            oimg.save(outPath)
            print outPath
