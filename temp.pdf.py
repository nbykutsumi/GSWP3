    ...
    for varName in lvarName:
        ...
        for season in lseason:
            ...
            for iprj, prjName in enumerate(lprjName):
                ...
                #-- mask too low temperature --
                if (prjName=='WFDEI')&(varName=='Tair')&(calctype=='mean'):
                    a2dat = ma.masked_less(a2dat, 220)
                #---------------------
                a1dat = a2dat.compressed()
 
                vmin, vmax = a1dat.min(), a1dat.max()
                bin_edges  = linspace(vmin,vmax,100)
                hist, bin_edges_tmp = np.histogram(a1dat, bins=bin_edges, density=True)
    
                x = 0.5*(bin_edges[:-1] + bin_edges[1:]) 
                col = {'GSWP3':'dodgerblue','PRINCETON':'orange','CRUNCEP':'green','WFDEI':'red'}
                ax.plot(x, hist, '-', label=prjName, color=col[prjName])
    
                #kde = scipy.stats.gaussian_kde(a1dat, bw_method='scott')
                #vmin, vmax = a1dat.min(), a1dat.max()
                #x   = np.linspace(vmin, vmax, 100)

                #col = {'GSWP3':'dodgerblue','PRINCETON':'orange','CRUNCEP':'green','WFDEI':'red'}
                #ax.plot(x, kde(x), label=prjName, color=col[prjName])
            ...
            figDir  = baseDir + "/fig"
            util.mk_dir(figDir)
            figPath = figDir + "/temp.pdf.%s.%s.%04d-%04d.%s.png"%(calctype, varName, iYear,eYear,season)
